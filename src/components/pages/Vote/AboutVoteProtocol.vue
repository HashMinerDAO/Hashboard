<template lang="html">
  <div class="flex-overview">
    <div class="ui-ele" v-if="!spaces || !snapshot">
      <router-link :to="{ name: 'vote' }" class="btn-link neutral subtle">
      ...
      <p></p>
      <LoadingEle :long="true" />
    </div>
    <div class="ui-ele" v-else>
      <router-link :to="{ name: 'vote' }" class="btn-link neutral subtle">
        <i class="i-arrow-left"></i>
        Back
      </router-link>
      ...
      <h1>
        <br />
        {{ proposals.length > 0 ? proposals.length : `No ` }} Proposal{{
    ...
    members() {
      ...
      if (this.space.members) {
        for (const i of this.space.members) {
          let address =
            i.toLowerCase() === this.wallet.toLowerCase()
              ? "You"
              : `${i.substr(0, 6)}...${i.substr(i.length - 6)}`;
